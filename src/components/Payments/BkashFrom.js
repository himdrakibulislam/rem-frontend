...
import { deleteAddonSettings, useAddonUpdateSettings } from "../../hooks/react-query/settings";
...
const BkashForm = ({ defaultValues, gateway, isBank = false }) => {
  ...
  const addonDeleteMutation = useMutation({
    mutationFn:  deleteAddonSettings,
    onSuccess: (response) => {
  ...
  const handleAddonDelete = () => {
    addonDeleteMutation.mutate({settings_type : gateway, _method: "DELETE" });
  }
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Box textAlign="right">
          ...
          onClick={handleOpen}
            sx={{
              mt: 2,
              backgroundImage: `linear-gradient(to right, ${defaultValues.theme_color}, ${defaultValues.theme_color})`, // Linear gradient
              color: "white",
              ":hover": {
                color: "white",
              },
      ...
      {open && (
        <CustomModal open={open} handleClose={handleClose}>
          <Typography fontWeight={700}>Are you sure you want to remove this?</Typography>
          <Button sx={{mr:2,mt:2}} onClick={handleClose}>No</Button>
          <Button sx={{mt : 2}} onClick={handleAddonDelete}>Yes</Button>
        </CustomModal>
      )}
    </>
  );
